...
def benchmark_diffnet(sij_generator, ntimes=100, optimalities=['D', 'A', 'Etree'], epsilon=1e-2):
    '''
    ...
    topo = dict([(o, [np.zeros(nbins, dtype=float), np.zeros(nbins, dtype=float), [], []]) for o in optimalities])
    nfails = 0
    for t in range(ntimes):
        ...
        for o in optimalities:
            ...
            nos = ssum * n / sij
            d = np.diag(nos)
            u = [nos[i, j] for i in range(n.shape[0]) for j in range(i + 1, n.shape[0])]
        ...
        results.update(dict(MSTn=MST_optimize(sij, 'n'), MSTs=MST_optimize(sij, 'std'), MSTv=MST_optimize(sij, 'var')))
        results.update(
            dict(cstn=const_allocation(sij, 'n'), csts=const_allocation(sij, 'std'), cstv=const_allocation(sij,
                                                                                                           'var')))
...
def benchmark_random_net(K=30, sii_offset=0., sij_min=1., sij_max=5., ntimes=100):
    def sij_generator():
...
def opts():
    ...
    parser.add_argument('-K', '--num-points', type=int, default=30, help='Number of points in the network.')
    parser.add_argument('-T',
                        '--num-times',
                        type=int,
                        default=100,
                        help='Number of times to run the benchmark to collect statistics.')
    parser.add_argument('--out-distance-net',
                        default=None,
                        help='Name of pickle file to write benchmark results for distance net.')
    parser.add_argument('--out-random-net',
                        default=None,
                        help='Name of pickle file to write benchmark results for random net.')
    parser.add_argument('--sii-offset', type=float, default=0., help='The average offset of s_{ii} from s_{ij}.')
    parser.add_argument('--sij-min', type=float, default=1., help='Minimum s_{ij}')
    parser.add_argument('--sij-max', type=float, default=5., help='Maximum s_{ij}')
    parser.add_argument('--out-uniform-net',
                        default=None,
                        help='Name of pickle file to write analysis results for uniform net.')
    parser.add_argument('--out-E-tree-timings',
                        default=None,
...
def main(args):
    ...
    if args.out_distance_net is not None:
        ...
        pickle.dump(dict(stats=stats, topo=topo), file(args.out_distance_net, 'wb'))
        write_average(avg)
    if args.out_random_net is not None:
        print('Benchmarking diffnet for random nets...')
        stats, avg, topo = benchmark_random_net(args.num_points,
                                                args.sii_offset,
                                                args.sij_min,
                                                args.sij_max,
                                                ntimes=args.num_times)
        pickle.dump(dict(stats=stats, topo=topo), file(args.out_random_net, 'wb'))
        write_average(avg)
