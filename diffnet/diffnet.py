...
def D_optimize(sij):
    ...
    def F(x=None, z=None):
        if x is None:
            x0 = matrix([sij[i, i] for i in range(K)] + [sij[i, j] for i in range(K) for j in range(i + 1, K)], (M, 1))
            return (0, x0)
        if z is None:
            return lndetC(sij, x)
...
OPTIMIZERS = dict(D=D_optimize, A=A_optimize, E=E_optimize, Etree=E_optimal_tree)


def optimize(sij, optimalities=['D', 'A', 'E', 'Etree'], cutoff=1e-4):
    '''
...
def check_MLest(K=10, sigma=0.1, noerr=True, disconnect=False):
    ...
    if (not disconnect):
        ...
        xML, vML = MLestimate(xij, invsij2, np.concatenate([x0[:disconnect + 1], [None] * (K - disconnect - 1)]))
...
def unitTest(tol=1.e-4):
    if (True):
        sij = matrix([[1.5, 0.1, 0.2, 0.5], [0.1, 1.1, 0.3, 0.2], [0.2, 0.3, 1.2, 0.1], [0.5, 0.2, 0.1, 0.9]])
    elif (False):
        sij = np.ones((4, 4), dtype=float)
        sij += np.diag(4. * np.ones(4))
        sij = matrix(sij)
    else:
        sij = matrix([[1., 0.1, 0.1], [0.1, 1., 0.1], [0.1, 0.1, 1.2]])
    ...
    for disconnect, label in [(False, 'Full-rank'), (1, 'No individual measurement'), (2, '2-disconnected')]:
